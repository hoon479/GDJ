...
public class MyController {
	...
	/*
	  	...
	  	@RequestMapping(value="/", method=RequestMapping.POST)
	  	
	  	Spring4부터 새로운 에너테이션으로 바꿀 수 있다.
	...
	/*
		...
		1. @Inject    
			...
			2) 동이 타입이 여러 개 있는 경우 @Qualifier를 이용해서 Bean을 식별
		2. @Recource   
	...
	*/
	...
	public void setBoard(Board board) {	  // 매개변수 Board로 Board 타입의 Bean이 Container에서 가져오거라
		this.board = board;
	}
	*/
	...
	//    2) 식별자는(@Qualifier)는 Bean의 이름(id)이 일치하는 Bean을 가져온다.
	...
	*/
	...
	// @Autowired를 사용하는 이유 : @Inject + @Qualfier
	/*
	 
	@Inject
	@Qualfier(value="board1")
	private Board b1;
	
	@Inject
	@Qualfier(value="board2")
	private Board b2;
