...
-- 기초 데이터 준비
-- HR 계정의 EMPLOYEES 테이블을 SCOTT 게정으로 복사해 오기
...
ALTER TABLE EMPLOYEES
         ADD CONSTRAINT PR_EMPLOYEES PRIMARY KEY(EMPLOYEE_ID);
...
BEGIN
       ...
       ELSIF GRADE >= 60 THEN
        ...
        ELSE
             RESULT := 'F';
        END IF;
        DBMS_OUTPUT.PUT_LINE(GRADE || '점은 ' || RESULT || '학점입니다.');
END;
...
EXCEPTION
         WHEN NO_DATA_FOUND THEN
            DBMS_OUTPUT.PUT_LINE('조회된 데이터가없음');
