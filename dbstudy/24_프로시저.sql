...
-- 프로시저 PROC2 정의
-- 사원번호가 = 100인 사원의 FRIST_NAME을 서버메시지로 출력하기
...
-- 프로시저 PROC4 정의
-- 사원번호가 = 100인 사원의 FRIST_NAME을 출력 파라미터 FNAME에 저장하기
...
BEGIN
     -- 1) 구매 테이블에 구매 내역을 INSERT 한다.
         ...
         VALUES(BUY_SEQ.NEXTVAL, C_NO, P_CODE, BUY_AMY);
     -- 2) 고객 테이블의 고객포인트를 UPDATE 한다. (총 구매액의 10% 적립, 정수로 올림 처리)  
      ...
      -- 3) 제품 테이블의 재고를 UPDATE 한다.
      UPDATE PRODUCT
         SET PROC_STOCK = PROC_STOCK - BUY_AMT
...
EXCEPTION
     ...
     WHEN OTHERS THEN
          ...
          DBMS_OUTPUT.PUT_LINE('예외메세지' || SQLERRM);
          -- 롤백
          ROLLBACK;
END BUY_PROC;

-- 구매 프로시저 호출
EXECUTE BUY_PROCC(1, 1000, 10);  -- 고객번호 1, 제품코드 1000, 구매수량 10
...
SELECT CUST_NO, CUST_NAME, CUST_POINT
   FROM CUSTOMER;
SELECT BUY_NO, CUST_NAME, RPOD_CODE, BUY_AMOUNT
FROM BUY;
