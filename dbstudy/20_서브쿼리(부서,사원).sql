...
SELECT ROWID, EMP_NO, NAME
   FROM EMPLOYEE
   WHERE ROWID = 'AAAFCPAABAAALDRAAC';
...
-- 3) RANK() 함수
--    정렬 후 순위 매기는 함수
--    목록 가져오기에서는 부적절(동점자 처리 떄문에 가져오는 목록의 수가 매번 달라질 수 있음) 
SELECT A.EMP_NO, A.NAME, A.SALARY
...
SELECT A.EMP_NO, A.NAME, A.HIRE_DATE
   FROM (SELECT ROW_NUMBER() OVER(ORDER BY HIRE_DATE ASC)AS ROW_NUM, EMP_NO, NAME, HIRE_DATE
            FROM EMPLOYEE) A
...
-- 2. 테이블 복사할 때 사용
-- 3. NOT NULL 제약조건 제외한 제약조건은 복사되지 않음
-- 4. 형식
