/*
1.
테이블 이름(TABLE_NAME)이 'EMP'인 테이블에 존재하는 인덱스의 이름(INDEX_NAME)과 해당 인덱스가 설정된 칼럼 이름을 조회할 수 있는 쿼리문을 작성하시오.
...
다음 지시사항의 대상 테이블과 대상 칼럼 정보를 이용해서 새로운 뷰(VIEW)를 생성하는 쿼리문을 작성하시오. (10점)
    ...
    3) 뷰 이름 : V_PRODUCT
*/ 
            CREATE VIEW_V.PRODUCT AS
            SELECT V.PRODUCT_NAME, V.PRODUCT_PRICE
                    FROM PRODUCT;
