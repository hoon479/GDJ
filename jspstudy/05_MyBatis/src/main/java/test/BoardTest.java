...
public class BoardTest {
	...
	public void 상세조회테스트() {
		...
		assertEquals("공지4", board.getTitle());
		*/
		
		// 4번 게시글을 가진 게시글이 있으면 테스트 성공, 아니면 실패
		Board board = BoardDao.getInstance().selectBoardByNo(10);
		assertNotNull(board);  // board가 not null 이면 테스트 성공
	...
	public void 게시글수정테스트() {
		...
		board.setBoardNo(11);  // 게시글삽입테스트 결과로 삽입된 게시글번호
	...
	public void 게시글삭제테스트() {
		// 삽입 테스트로 삽입한 게시글 삭제
		int result = BoardDao.getInstance().deleteBoard(11);
		assertEquals(1, result);
	}
